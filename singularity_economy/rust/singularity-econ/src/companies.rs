...
//! multiples are analytic judgments; mcap/NTM come from the phase-2
//! verified financials and can be refreshed via `load_financials`.
...
pub fn universe() -> Vec<Company> {
    ...
    vec![
        ...
        c("ASML", "ASML", 692.5, 14.0, vec![(R::Silicon, 1.0)],
          0.9, 0.01, 22.0, Long, vec![],
          "Litho monopoly; IP-moat rent class; scale in toward ~35x"),
        // ---- LONGS: power / grid ----
        c("GEV", "GE Vernova", 274.0, 8.5, vec![(R::PowerEquipment, 1.0)],
          1.1, 0.01, 18.0, Long, vec![],
          "Sold out through 2030 (killed at price by red team; watchlist $700-800)"),
        c("VST", "Vistra", 65.0, 3.8, vec![(R::GdpIndex, 1.0)],
          0.4, 0.0, 14.0, Long, vec![(E::Electricity, 0.05, 0.5)],
          "Merchant torque to power scarcity"),
        c("NRG", "NRG Energy", 35.0, 2.4, vec![(R::GdpIndex, 1.0)],
          0.4, 0.01, 13.0, Long, vec![(E::Electricity, 0.045, 0.45)],
        ...
        c("TCS.NS", "Tata Consultancy", 95.0, 6.0, vec![(R::ItServices, 1.0)],
          1.3, 0.0, 10.0, Short, vec![],
          "KILLED high conf (thesis-incoherent: they sell the integration)"),
        c("WDAY", "Workday", 31.6, 2.4, vec![(R::SeatSaas, 1.0)],
          1.2, 0.0, 11.0, Short, vec![],
          "KILLED as delta-one; trigger-gated put spreads (cRPO <10%)"),
        c("TEAM", "Atlassian", 20.3, 1.4, vec![(R::SeatSaas, 1.0)],
          1.3, -0.02, 11.0, Short, vec![],
          "KILLED high conf (capture-ratio category error)"),
        c("HUBS", "HubSpot", 9.7, 0.65, vec![(R::SeatSaas, 1.0)],
          1.3, -0.02, 11.0, Short, vec![],
          "Live short, small, kill-watch (outcome-pricing pivot)"),
        c("FDS", "FactSet", 12.0, 0.60,
          vec![(R::SeatSaas, 0.5), (R::ProfInfo, 0.5)],
          1.0, -0.01, 12.0, Short, vec![],
          "KILLED by red team"),
        c("MMC", "Marsh McLennan", 95.0, 4.6,
          vec![(R::ProfInfo, 0.8), (R::GdpIndex, 0.2)],
          1.0, 0.0, 14.0, Short, vec![],
          "KILLED by red team"),
        c("CHRW", "CH Robinson", 18.0, 0.75,
          vec![(R::HumanPhysicalWages, 0.5), (R::GdpIndex, 0.5)],
          1.0, -0.05, 11.0, Short, vec![],
          "Live short post-print ($210-225 entry)"),
        c("LSTR", "Landstar", 6.0, 0.30,
          vec![(R::HumanPhysicalWages, 0.6), (R::GdpIndex, 0.4)],
          1.0, -0.04, 10.0, Short, vec![],
          "KILLED by red team"),
        c("MRVL", "Marvell", 200.0, 3.2, vec![(R::Silicon, 1.0)],
          1.0, -0.08, 13.0, Short, vec![],
          "Pair vs AVGO, half size, enter on relief"),
        c("6954.T", "FANUC", 44.0, 1.5, vec![(R::GdpIndex, 1.0)],
          0.9, -0.02, 18.0, Short, vec![(E::Robots, 0.05, 0.20)],
          "KILLED (fair with humanoid credit)"),
        c("6324.T", "Harmonic Drive", 4.43, 0.031, vec![(R::GdpIndex, 1.0)],
          1.0, 0.0, 25.0, Short, vec![(E::RobotComponents, 0.06, 0.28)],
          "KILLED outright (squeeze); pair-only revival"),
        c("EQIX", "Equinix", 75.0, 2.6,
          vec![(R::DcInfra, 0.3), (R::GdpIndex, 0.7)],
          0.5, -0.01, 16.0, Short, vec![],
          "KILLED by red team"),
        c("PLTR", "Palantir", 323.3, 4.0,
          vec![(R::SeatSaas, 0.3), (R::GdpIndex, 0.7)],
          1.1, 0.0, 25.0, Short, vec![(E::AiServices, 0.015, 0.30)],
